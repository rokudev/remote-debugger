...
class HTTPResponse(object):
    ...
    def __init__(self, socket, debugLevel=0):
        self.mDebug = debugLevel
        self.mSocket = socket
        self._readResponse()
...
class HTTPConnection(object):
    ...
    def __init__(self, netLoc, debugLevel=0):
        self.mDebug = debugLevel
        parts = netLoc.split(':')
        self.mHost = parts[0]
        if len(parts) > 1:
    ...
    def send(self, data):
        if self.mDebug >= 5:
            ...
            DebugUtils.dumpBytes(data, forceEol=True, maxLen=maxLen)
    ...
    def getResponse(self):
        return HTTPResponse(self.mSocket, self.mDebug)
    ...
    def debugDumpRequest(self, headers, bodyData):
        ...
        from .DebugUtils import DebugUtils
        DebugUtils.dumpBytes(bodyData, label='BODY', forceEol=True)
        print('^^^^^^^^^^ debug:REQUEST ^^^^^^^^^^')
